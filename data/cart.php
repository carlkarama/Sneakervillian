...
//echo "loading...";
	...
	if (isset($_GET['add'])) {
		...
		while($row = mysqli_fetch_array($result)) {
			...
			if($row['quantity'] != $_SESSION['productID' . $item]) {
				...
				echo "<script>location.replace('../data/checkout.php')</script>";
				...
				//session_destroy();
				echo "<script>location.replace('../admin/index_admin.php?stock=oos')</script>";
				}
	...
	if (isset($_GET['remove'])) {
		...
		if ($_SESSION['productID' . $remove_item] < 1) {
			...
			echo "<script>location.replace('../data/checkout.php?item_msg=air')</script>";

			if ($_SESSION['productID' . $remove_item] = 0) {

				echo "<script>location.replace('../data/checkout.php?item_msg=z')</script>";
			}
		}

		else {

			echo "<script>location.replace('../data/checkout.php?item_msg=ir')</script>";
		}
	}
	...
	if (isset($_GET['delete'])) {
		...
		echo "<script>location.replace('../data/checkout.php?item_msg=id')</script>";
	}
			...
			if ($value > 0) {
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					$itemQuantity += $value; 
						...
						echo '<td>' . $value . '</td>'; 
						echo '<td>&#36;' . $sub . '</td>';
						echo "<td id='cart-add'><a href='../data/cart.php?add={$row['productID']}'><img src='stock/act/add.png' height='30' width='30'></a></td>";
						echo "<td id='cart-remove'><a href='../data/cart.php?remove={$row['productID']}'><img src='stock/act/remove.png' height='30' width='30'></a></td>";
						echo "<td id='cart-delete'><a href='../data/cart.php?delete={$row['productID']}'><img src='stock/act/delete.png' height='30' width='30'></a></td>";
						echo "</tr>";
